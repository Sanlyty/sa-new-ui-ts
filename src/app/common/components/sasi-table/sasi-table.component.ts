...
} from "./row-group-table/row-group-table.component";
...
export class SasiRow {
  ...
  getCellValue(columnIndex: SasiColumn): any {
    return this.getCell(columnIndex.index)?.value;
  }
...
export class SasiTableOptions {
  ...
  getColumnWidth(name) {
    // TODO should be part of the SasiTableOptions but Object.assign will not copy it
  ...
  getAggregatedColumns(): SasiColumn[] {
    return this.columns.filter((column) => column.isAggregated);
  }

  getDataColumns(): SasiColumn[] {
    return this.columns.filter(
      (column) => column.index !== "name" && column.hidden !== true
    );
  }
...
export const slideInOutAnimation = [
  // TODO reuse animation in all collapsed sasi-table group
  trigger("slideInOut", [
...
export class SasiTableComponent implements OnInit, OnChanges {
  ...
  static isSelected(
    ...
    rowGroup: SasiGroupRow
  ) {
    return (
      selectedRow.rowName === row.getCell("name").value &&
      selectedRow.groupName === rowGroup.groupRow.getCell("name").value
    );
  }
  ...
  async ngOnInit() {
    ...
    this.localStorageService
      .observe(this.options.storageNamePrefix + "_collapsed")
      .subscribe((data) => {
        this.collapsedRows = data.newValue;
      });
    this.selectedRows = await this.localStorageService.get(
      this.options.storageNamePrefix + "_selected"
    );
    this.collapsedRows = this.localStorageService.get(
      this.options.storageNamePrefix + "_collapsed"
    );
    ...
    this.style = this.domSanitizer.bypassSecurityTrustStyle(
      "grid-template-columns: " +
        this.getColControlSize() +
        " " +
        this.getAlertColumnSize() +
        " " +
        this.getNameColumnSize() +
        " repeat(" +
        this.getGridColumnCount() +
        ", 1fr);"
    );
  ...
  private sortData(changes) {
    if (
      ...
      this.options.sortColumnNames !== undefined
    ) {
  ...
  private getSortColumns() {
    return this.options.sortColumnNames.map((sortColumn) =>
      this.options.columns.find((column) => column.index === sortColumn)
    );
  }
  ...
  getColControlSize() {
    return this.options.colControlFormatter != null ||
      this.options.selectableRows
      ? this.options.controlColumnSize
      : "";
  }

  getAlertColumnSize() {
    return this.options.isDataGrouped &&
      this.options.cellDecoratorRules.length > 0
      ? this.options.alertColumnSize
      : "";
  }

  getColumnLabel(type: string) {
    const column: SasiColumn = this.options.columns.find(
      (optionColumn) => optionColumn.index === type
    );
    if (column === undefined) {
      return "";
    }
  ...
  getColumnTooltipText(type: string) {
    ...
    return column.columnTooltipText !== null
      ? column.columnTooltipText
      : this.getColumnLabel(type);
  }
      ...
      d.forEach(
        (value) =>
          (this.collapsedRows = this.collapsedRows.filter(
            (collapsedRowValue) =>
              collapsedRowValue !== value.groupRow.getCell("name").value
          ))
      );
    } else {
      d.forEach((value) =>
        this.collapsedRows.push(value.groupRow.getCell("name").value)
      );
    }
  ...
  isSelectedAll(): boolean {
    if (!this.options.isDataGrouped) {
      ...
      return this.data.every((row) =>
        this.selectedRows.find(
          (selectedRow) => row.getCell("name").value === selectedRow.rowName
        )
      );
    }
    // @ts-ignore
    ...
    return d.every(
      ...
      (rowGroup) =>
        rowGroup.rows.every(
          (row) =>
            this.selectedRows.find((selectedRow) =>
              SasiTableComponent.isSelected(row, selectedRow, rowGroup)
            ) !== undefined
        )
    );
  }
  ...
  selectAll() {
    ...
    if (!this.isSelectedAll()) {
      this.onSelectService.announceSelectAll({
        operation: true,
        prefix: this.options.storageNamePrefix,
      });
    } else {
      this.onSelectService.announceSelectAll({
        operation: false,
  ...
  getHeaderGridStyle(i: number) {
    ...
    const startPosition = groupsBefore.reduce(
      (previousValue, currentValue) =>
        previousValue + currentValue.columns.length,
      offsetPosition
    );
