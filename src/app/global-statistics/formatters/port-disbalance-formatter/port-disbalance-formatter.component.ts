...
export class PortDisbalanceFormatterComponent implements OnInit {
  ...
  private imbalanceEvents = "PORT_IMBALANCE_EVENTS";

  static isVisible(row) {
    if (
      row !== undefined &&
      row.getCell("PORT_IMBALANCE_EVENTS") !== null &&
      parseInt(row.getCell("PORT_IMBALANCE_EVENTS").value, 10) > 0
    ) {
      return true;
    }
    return false;
  ...
  ngOnInit() {
    if (this.rowData !== undefined) {
      this.rowData.subRows
        .filter((subRow) => PortDisbalanceFormatterComponent.isVisible(subRow))
        .forEach((subRow) => this.subData.push(subRow));
    }
  }
  ...
  private resolveAbsoluteDisbalance() {
    if (
      ...
      this.rowData.getCell("PORT_IMBALANCE_ABSOLUT") != null
    ) {
      return this.rowData.getCell("PORT_IMBALANCE_ABSOLUT").value;
    }
  }
