      ...
      (data) => {
        ...
        this.data.forEach((system) =>
          system.children.forEach(
            (pool) =>
              (pool.children = pool.children.filter(
                (port) => port.metrics.length > 0 && port.metrics[0].value > 0
              ))
          )
        );
        this.data.forEach(
          (system) =>
            (system.children = system.children.filter(
              (pool) =>
                (pool.metrics.length > 0 && pool.metrics[0].value > 0) ||
                pool.children.length > 0
            ))
        );
      },
