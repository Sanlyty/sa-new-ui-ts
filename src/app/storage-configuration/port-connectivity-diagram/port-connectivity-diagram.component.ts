...
export class PortConnectivityDiagramComponent implements OnInit {
  ...
  public static getContent(node: NodeModel): HTMLElement {
    ...
    if (node.data != null && (node.data as Owner).detail != null) {
      const detail = (node.data as Owner)
        .detail as StorageEntityDetailResponseDto;
      ...
      tooltipContent.innerHTML =
        '<div style="background-color: #2f2f2f; color: #bfbfbf; border-width:1px;border-style: solid;border-color: #d3d3d3; border-radius: 8px;white-space: nowrap;"><ul style="list-style: none; padding: 2px">' +
        content +
        "</ul></div>";
      if (content === "") {
        tooltipContent = null;
  ...
  ngOnInit() {
    this.route.params.subscribe((params) => {
      ...
      if (params["id"] != undefined) {
        ...
        this.metricService
          .getStorageEntityDetail(StorageEntityType.PORT, this.selectedSystem, [
  ...
  public nodeDefaults(node: NodeModel): NodeModel {
    ...
    node.annotations = [
      {
        content: (node.data as StorageEntitDiagramData).name,
        style: { color: "black" },
      },
    ];
    node.shape = {
