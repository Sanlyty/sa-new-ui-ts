...
export class StorageEntityDetail2SasiTablePipe implements PipeTransform {
  transform(
    ...
    return systems.map((system) => {
      ...
      row.cells["status"] = new SasiCell(ComponentStatus[system.status], {
        value: ComponentStatus[system.status],
      });
      row.cells["serialNumber"] = new SasiCell(system.serialNumber, {
        value: system.serialNumber,
      });
      row.cells["lastDataUpdate"] = new SasiCell(lastDataUpdate[system.id], {
        value: lastDataUpdate[system.id],
      });
      if (system.detail !== undefined) {
        const detail = system.detail;
        row.cells["arrayModel"] = new SasiCell(detail.arrayModel, {
          value: detail.arrayModel,
        });
        row.cells["managementIp"] = new SasiCell(detail.managementIp, {
          value: detail.managementIp,
        });
        ...
        row.cells["prefixReferenceId"] = new SasiCell(
          detail.prefixReferenceId,
          { value: detail.prefixReferenceId }
        );
        row.cells["sortId"] = new SasiCell(detail.sortId, {
          value: detail.sortId,
        });
        row.cells["speed"] = new SasiCell(detail.speed, {
          value: detail.speed,
        });
        row.cells["note"] = new SasiCell(detail.note, { value: detail.note });
        row.cells["cables"] = new SasiCell(detail.cables, {
          value: detail.cables,
        });
        row.cells["switch"] = new SasiCell(detail.switch, {
          value: detail.switch,
        });
